...
Simon UcedaVelez (sau8), Diego Fernandez (df365)
11/2/2020
"""
...
def multi_polygons(w, side, k, n, sp):
    ...
    assert is_valid_length(side), \
    report_error('side is not a valid length',side)
    assert is_valid_iteration(k), \
    report_error('k is not a valid number of polygons',k)
    assert (type(n)==int and n>=3), \
    report_error('n is not a valid number of sides',n)
    assert is_valid_speed(sp), report_error('sp is not a valid speed',sp)
...
def multi_polygons_helper(t, side, k, n, sp):
    ...
    report_error('side is not a valid length',side)
    assert is_valid_iteration(k), \
    report_error('k is not a valid number of iterations',k)
    assert (type(n)==int and n>=3), \
    report_error('n is not a valid number of polygons',n)
    assert is_valid_speed(sp), report_error('sp is not a valid speed',sp)
...
def draw_polygon(t, side, n):
    ...
    report_error('n is an invalid # of poly sides',n)
...
def draw_diamond(t, length, width):
    ...
    Precondition: width is a valid side length (number >= 0)
    """
    assert is_valid_turtlemode(t), \
    report_error('Invalid turtle mode', t)
...
def triangle(w, side, d, sp):
    ...
    assert is_window(w), report_error('w is not a valid window',w)
    assert is_valid_length(side), \
    report_error('side is not a valid side length',side)
...
def triangle_helper(p, x, y, side, d):
    ...
    if d==0:
        ...
        triangle_helper(p, x+(side/4), y+((3)**(0.5)*(side/4.0)), side/2.0, d-1)
        triangle_helper(p, x+(side/2), y, side/2.0, d-1)
...
def cantor(w, side, hght, d, sp):
    ...
    assert is_valid_length(side), \
    report_error('side is not a valid side length',side)
    assert is_valid_length(hght), \
    report_error('hght is not a valid height length',hght)
    assert is_valid_depth(d), \
    report_error('d is not a valid depth',d)
    assert is_valid_speed(sp), \
    report_error('sp is not a valid turtle/pen speed',sp)
    #clears Window
    w.clear()
    ...
    #sets both edge and fill pen colors to be red
    p.fillcolor = 'red'
    p.edgecolor = 'red'

    #makes the pen visible during drawing
    p.visible = True

    cantor_helper(p, 0, 0, side, hght, d)

    #makes the pen invisible during drawing
    p.visible = False
...
def cantor_helper(p, x, y, side, hght, d):
    ...
    report_error('side is not a valid length',side)
    assert is_valid_length(hght), \
    report_error('hght is not a valid length',hght)
    assert is_valid_depth(d), report_error('d is not a valid depth',d)

    if(d == 0):
        fill_rect(p, x, y, side, hght)
    # recursive case where d is greater than 0
    else:
        fill_rect(p, x, y + hght/4, side, hght/2)
        cantor_helper(p, x - side/3, y -hght/4, side/3, hght/2, d-1)
        cantor_helper(p, x + side/3, y -hght/4, side/3, hght/2, d-1)
