...
class Image(object):
    ...
    def setWidth(self,value):
        ...
        """
        assert type(value) == int, 'value is not a valid type'
        assert value > 0, 'value is not withon range'
        assert len(self.getData())%value == 0, 'value does not evenly divide \
        columns and rows'

        self._width = value
        #adjust height
        h = len(self.getData())//value
        self._height = h
        # Implement me
    ...
    def setHeight(self,value):
        ...
        """
        assert type(value) == int, 'value is not a valid type'
        assert value > 0, 'value is not a valid type'
        assert len(self.getData())%value == 0, 'value does not evenly divide \
        columns and rows'

        self._height = value
        #adjust width
        w = len(self.getData())//value
        self._width = w
        # Implement me
    ...
    def __init__(self, data, width):
        ...
        assert data != [], 'data is an empty pixel list'
        assert type(width) == int, 'width is not an int'
        assert width > 0, 'width is not a valid int value'
        assert len(data)%width  == 0, 'width is not a valid value for data'
    ...
    def __len__(self):
        ...
        """
        return len(self._data)   # Implement me
    ...
    def __getitem__(self, pos):
        ...
        """
        assert type(pos) == int, 'the value pos is not an int'
        assert pos >=0, 'pos is not in the correct range of ints \
        0<=pos<=len(len(self._data)'
        assert pos < len(self._data), 'pos is not in the correct range \
        of ints 0<=pos<=len(len(self._data)'
    ...
    def __setitem__(self, pos, pixel):
        ...
        """
        assert type(pos) == int, 'the value pos is not an int'
        assert pos >=0, 'pos is not in the correct range of ints >=0'
        assert pos < len(self._data), 'pos is not in the correct range of \
        ints 0<=pos<=len(len(self._data)'
        assert _is_pixel(pixel), 'the pixel provided is not an acceptable \
        pixel value'
    ...
    def getPixel(self, row, col):
        ...
        """
        assert type(row) == int, 'row is not a not a valid int value'
        assert row >= 0 and row < self.getHeight(), 'row is not within the \
        valid range 0 <= row < height'
        assert type(col) == int, 'col is not a not a valid int value'
        assert col >= 0 and col < self.getWidth(), 'col is not within the \
        valid range 0 <= col < width'

        width = self.getWidth()
        pullValue = width * row + col
        #used ._data instead of getData() to make all other functions faster
        return self._data[pullValue]
        # Implement me
    ...
    def setPixel(self, row, col, pixel):
        ...
        self._data[pullValue] = pixel    # Implement me
    ...
    def __str__(self):
        ...
        for i in range(self.getHeight()):
            ...
            for j in range(self.getWidth()):
                rowList = rowList + str(self.getPixel(i,j))
                if j < (self.getWidth()-1):
                    rowList = rowList + ', '
            newList = newList + '[' + rowList + ']'
            rowList = ''

            if i in range(self.getHeight()-1):
                newList = newList + ',\n'
    ...
    def copy(self):
        ...
        """
        copied = Image(self.getData(), self.getWidth())
        return copied
        # Implement me
